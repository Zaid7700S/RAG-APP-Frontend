...
export default function ChatWindow({ 
  t, chatHistory, loadingChat, chatEndRef, isSidebarOpen, setIsSidebarOpen 
}) {
  return (
    <>
      <div style={{ padding: '0.8rem 1rem', display: 'flex', alignItems: 'center', minHeight: '40px' }}>
        {!isSidebarOpen && (
          <button onClick={() => setIsSidebarOpen(true)} style={{ background: 'none', border: 'none', color: t.textMuted, cursor: 'pointer' }}>
            <Menu size={24} />
          </button>
        )}
      </div>
